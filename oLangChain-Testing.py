...
""".strip()
    ...
    def predict(self, X: pd.DataFrame, y_true: pd.Series, max_retries=3) -> np.ndarray:
        ...
        for _, row in train_sample.iterrows():
            # If possible, use the column names (or use feature indices if none exist)
            if row.index.dtype == np.int64:
                features_str = ", ".join([f"Feature_{idx}: {val}" for idx, val in enumerate(row.iloc[:-1])])
            else:
                features_str = ", ".join([f"{col}: {val}" for col, val in row.iloc[:-1].iteritems()])
            label = row.iloc[-1]
            q_line = f"Q: {features_str}"
            a_line = f"A: {label}"
...
class ChromaVectorStoreManager:
    ...
    def build_vectorstore(self, X_train: pd.DataFrame, y_train: pd.Series):
        ...
        for i in range(len(X_train)):
            # Build a detailed representation by including feature names.
            # If X_train has column names use them; otherwise, use index numbers.
            row_series = X_train.iloc[i]
            if X_train.columns.isnull().any():
                features_str = ", ".join([f"Feature_{idx}: {val}" for idx, val in enumerate(row_series)])
            else:
                features_str = ", ".join([f"{col}: {row_series[col]}" for col in X_train.columns])
            label_str = str(y_train.iloc[i])
            content = f"{features_str}. Label: {label_str}"
...
Follow these rules:
1) First, generate a brief chain-of-thought explanation that outlines your reasoning.
2) Return the final answer ONLY in JSON format.
3) Do not include any extra text, markdown, or code fences.

Below are training examples with feature details:
{retrieved_examples}

Now, classify the following new data point described as:
{test_case}

Return valid JSON exactly in this format:
{{ "classification": <integer>, "reasoning": "<your brief explanation including chain-of-thought>" }}
""".strip()
