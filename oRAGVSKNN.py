...
if __name__ == '__main__':
    ...
    'zoo']
    n_splits = 2
    methods = ['RAG', 'RAG_no_embeddings', 'KNN']
    k_neighbors = 2
        ...
        if MODE == 'single':
            for ds, fold, method, X_tr, y_tr, X_te, y_te in tasks:
                if method == 'KNN':
                    ...
                    gpu_id = (fold - 1) % gpu_count
                    if method == 'RAG_no_embeddings':
                        cls = LangChainOllamaRAGNoEmbeddingsClassifier(
                        ...
                        cls = LangChainOllamaRAGClassifier(
                            k_neighbors=k_neighbors,
                            persist_dir=f"chroma_db/{ds}_f{fold}/{method}",
                            cuda_device=gpu_id
                        )
            ...
            cpu_tasks = [t for t in tasks if t[2] == 'KNN']
            gpu_tasks = [t for t in tasks if t[2] != 'KNN']
            ...
            with ProcessPoolExecutor(max_workers=gpu_count) as gpu_exec:
                for idx, (ds, fold, method, X_tr, y_tr, X_te, y_te) in enumerate(gpu_tasks):
                    gpu_id = idx % gpu_count
                    gpu_exec.submit(run_rag_task, ds, fold, method, X_tr, y_tr, X_te, y_te, k_neighbors, gpu_id)
